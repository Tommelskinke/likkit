...
const testTasks: Task[] = [
  ...
  { id: 2, title: 'Møt opp på forelesning', description: 'hei', done: false },
...
describe('Create new task (POST)', () => {
  test('Create new task (200 OK)', (done) => {
    axios.post('/tasks', { title: 'Ny oppgave', description: 'morsomt' }).then((response) => {
      expect(response.status).toEqual(200);
...
describe('Update task (PUT)', () => {
  test('Update task (200 OK)', (done) => {
    axios
      .put('/tasks', { id: 3, title: 'Gjør øving', description: 'jajaja', done: false })
      .then((response) => {
        expect(response.status).toEqual(200);
        done();
      });
